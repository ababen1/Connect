...
func _create_raycast(cell: Vector2) -> PathRaycast:
	...
	raycast.global_position = grid.to_global(
		grid.map_to_local(cell)) + grid.cell_size / 2
...
func raycast_to_cells(raycast: PathRaycast) -> PackedVector2Array:
	var cells: = PackedVector2Array([])
	var start_point = grid.local_to_map(raycast.global_position)
	var end_point = grid.local_to_map(raycast.global_position + raycast.target_position)
...
static func are_parallel(vector1: Vector2, vector2: Vector2) -> bool:
	var angle = int(rad_to_deg(vector1.angle_to(vector2)))
...
func _find_possible_paths(start: Vector2, end: Vector2) -> Array:
	...
	var direct_path: PathData = _find_direct_path(raycast_start, raycast_end)
	if direct_path:
	...
	for direction_start in DIRECTIONS:
		for direction_end in DIRECTIONS:
			raycast_start.target_position = direction_start * grid.cell_size
			raycast_end.target_position = direction_end * grid.cell_size
			raycast_start.force_raycast_update()
...
func _find_path_with_parallel_raycasts(
	raycast_start: PathRaycast, 
	raycast_end: PathRaycast) -> PathData:
		var start_direction = raycast_start.target_position.normalized()
		var end_direction = raycast_end.target_position.normalized()
		...
		for long_cell in longer_cast_cells:
			for short_cell in shorter_raycast_cells:
				...
				if connecting_raycast and not connecting_raycast.is_colliding() and (
					...
					connecting_raycast.target_position.normalized() in DIRECTIONS)))):
						return PathData.new(
							longer_raycast, 
							shorter_raycast, 
							connecting_raycast)
...
func _create_raycast_between_cells(start: Vector2, end: Vector2) -> PathRaycast:
	...
	if direction in DIRECTIONS:
		var distance = new_raycast.global_position.distance_to(
			grid.map_to_local(end) + grid.cell_size / 2)
		new_raycast.target_position = direction * distance
...
func _can_extend_cast(raycast: PathRaycast, extend_by: Vector2) -> bool:
	return grid.get_rect_world().has_point(raycast.get_casting_global_pos() + extend_by) and (
		...
		grid.get_cellv(grid.local_to_map(raycast.get_casting_global_pos() + extend_by)) == grid.INVALID_CELL)

func _extend_cast(raycast: PathRaycast, extend_by: Vector2) -> void:
	if _can_extend_cast(raycast, extend_by):
		raycast.target_position += extend_by
...
func _find_path_with_perpendicular_raycasts(
	raycast_start: PathRaycast, raycast_end: PathRaycast) -> PathData:
		var start_direction = raycast_start.target_position.normalized()
		var end_direction = raycast_end.target_position.normalized()
		...
		if not raycast_start.is_colliding() and not raycast_end.is_colliding():
			...
			if collision_point:
				return PathData.new(raycast_start, raycast_end)
		return null
